...
int main() {
    ...
    p.addComment("Nice post!", "sara");
    p.addComment("Great!", "ali");
    qDebug() << "Total comments:" << p.getComments().size();

    // Edit test
    p.editComment(0, "Updated!", "sara");
    qDebug() << "After edit:" << p.getComments()[0].getContent();

    // Delete test
    p.deleteComment(0, "sara");
    qDebug() << "After delete:" << p.getComments().size();
    ...
    // Load test
    p.loadCommentsFromFile();
    qDebug() << "Loaded:" << p.getComments().size();
