...
static QLabel* avatar(const QString& initial, int size, QWidget* p = nullptr)
{
    ...
    l->setStyleSheet(QString(
        "background:#202040; border-radius:%1px;"
        "font-size:%2px; font-weight:700; color:#7070FF;")
        .arg(size / 2).arg(size / 3));
    return l;
}
...
PostCard::PostCard(Posts* post, const QString& authorUsername,
    ...
    : QFrame(parent), m_post(post),
    m_authorUsername(authorUsername), m_isOwner(isOwner)
{
    ...
    headerRow->addWidget(avatar(initial, 36));
    ...
    headerRow->addLayout(col);
    headerRow->addStretch();

    if (m_isOwner) {
        auto* delBtn = makeIcon("✕");
        delBtn->setStyleSheet(
            "QPushButton{background:transparent;border:none;color:#553333;font-size:12px;}"
    ...
    likeCnt->setStyleSheet("color:#44446A; font-size:12px;");
    auto* cmtBtn = makeIcon("✦");
    auto* cmtCnt = new QLabel(QString::number(post->getComments().size()));
    cmtCnt->setStyleSheet("color:#44446A; font-size:12px;");
    ...
    actRow->addWidget(likeCnt);
    actRow->addSpacing(10);
    actRow->addWidget(cmtBtn);
    actRow->addWidget(cmtCnt);
    actRow->addStretch();
    ...
    : QWidget(parent), m_userCount(0), m_isLoginMode(true)
{
    ...
    m_stack->addWidget(createLoginWidget());   // 0
    m_stack->addWidget(createSignupWidget());  // 1
...
void AuthPage::resetToLogin()
{
    ...
    // Reload users so a newly registered user can login after switch
    m_userCount = 0;
    loadAllUsers(m_allUsers, m_userCount);
}
...
QWidget* AuthPage::createLoginWidget()
{
    ...
    vl->addWidget(makeLabel("Welcome back", "authTitle"));
    vl->addSpacing(4);
    ...
    m_loginPass->setFixedHeight(42);
    vl->addWidget(m_loginPass);

    vl->addSpacing(4);
    auto* loginBtn = makePrimary("Sign In");
    ...
    auto* lbl = new QLabel("No account?");
    lbl->setStyleSheet("color:#34345A; font-size:12px;");
    auto* btn = new QPushButton("Create one");
    btn->setCursor(Qt::PointingHandCursor);
    btn->setStyleSheet(
        "QPushButton{background:transparent;border:none;color:#5555CC;"
        "font-size:12px;text-decoration:underline;}"
        "QPushButton:hover{color:#8888FF;}");
    connect(btn, &QPushButton::clicked, this, &AuthPage::toggleMode);
    switchRow->addStretch();
    switchRow->addWidget(lbl);
    switchRow->addSpacing(5);
    switchRow->addWidget(btn);
    switchRow->addStretch();
    vl->addLayout(switchRow);
    ...
    if (!checker.checkAll(pass.toStdString())) {
        ...
        QMessageBox::warning(this, "Weak Password",
            QString("Password strength: %1 / 100\n\n"
                ...
                "  • One digit\n"
                "  • One special character").arg(strength));
        return;
    }
    ...
    : QWidget(parent), m_user(currentUser), m_allUsers(allUsers), m_userCount(userCount)
{
    ...
    // Compose card
    auto* compCard = new QFrame;
    compCard->setObjectName("postCard");
    auto* compLayout = new QVBoxLayout(compCard);
    compLayout->setContentsMargins(18, 14, 18, 14);
    ...
    QString ini = currentUser
        ...
        : "?";
    compRow->addWidget(avatar(ini, 34));
    compRow->addSpacing(10);
    m_postInput = new QTextEdit;
    m_postInput->setPlaceholderText("What's on your mind?");
    m_postInput->setFixedHeight(68);
    compRow->addWidget(m_postInput);
    ...
    compLayout->addLayout(postBtnRow);
    outer->addWidget(compCard);
...
void FeedPage::loadPosts()
{
    ...
    for (int u = 0; u < m_userCount; ++u) {
        ...
        for (int i = fpc - 1; i >= 0; --i) {
            ...
            auto* card = new PostCard(p, funame, false);
            connect(card, &PostCard::likeClicked, this, &FeedPage::onLikePost);
            connect(card, &PostCard::commentClicked, this, &FeedPage::onCommentPost);
            connect(card, &PostCard::deleteClicked, this, &FeedPage::onDeletePost);
            m_feedLayout->insertWidget(m_feedLayout->count() - 1, card);
    ...
    if (!any) {
        auto* empty = new QLabel("No posts yet — create one or follow someone!");
...
void FeedPage::onLikePost(Posts* post) { if (post) post->likePost(); }
    ...
    : QWidget(parent), m_user(currentUser)
{
    ...
    auto* hRow = new QHBoxLayout;
    hRow->addWidget(makeLabel("Notifications", "pageTitle"));
    hRow->addStretch();
    auto* refBtn = makeSecondary("↻  Refresh");
    refBtn->setFixedWidth(105);
...
void NotificationsPage::loadNotifications()
{
    ...
    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while (!in.atEnd()) {
            ...
            Notification notif(parts[0].toStdString(),
                parts[1].toStdString(),
                parts[2].toStdString());
    ...
    m_foundUser(nullptr), m_followBtn(nullptr)
{
    ...
    m_resultsLayout->setContentsMargins(0, 0, 6, 0);
...
void SearchPage::showUserCard(User* user)
{
    ...
    topRow->addWidget(avatar(ini, 50));
    ...
    auto addStat = [&](const QString& val, const QString& lbl) {
        ...
        v->setStyleSheet("font-size:19px; font-weight:700; color:#6060FF;");
    ...
    if (user->getIsReported()) {
        ...
        warn->setStyleSheet("color:#AA4040; font-size:12px; "
            "background:#180606; border-radius:6px; padding:6px 10px;");
        cl->addWidget(warn);
    }
...
void SearchPage::onFollowUser()
{
    ...
    if (m_currentUser->isFollowing(m_foundUser->getUsername())) {
        // unfollowUser already calls removeFollower internally — do NOT call it again
    ...
    : QWidget(parent), m_user(currentUser), m_allUsers(allUsers), m_userCountPtr(userCount)
{
    ...
    QString ini = currentUser
        ? QString::fromStdString(currentUser->getUsername()).left(1).toUpper()
        : "?";
    infoRow->addWidget(avatar(ini, 64));
    ...
    bcl->setContentsMargins(22, 18, 22, 18);
    bcl->setSpacing(10);
    ...
    playout->setContentsMargins(22, 18, 22, 18);
    ...
    savePassBtn->setFixedWidth(150);
    connect(savePassBtn, &QPushButton::clicked, this, &ProfilePage::onUpdatePassword);
    auto* passRow2 = new QHBoxLayout;
    passRow2->addStretch(); passRow2->addWidget(savePassBtn);
    ...
    m_currentUser(nullptr), m_allUsers(nullptr), m_userCount(0)
{
    ...
    // Central widget holds auth page AND app shell stacked on top of each other
    // using a QStackedWidget so they never overlap
    ...
    // appShell starts hidden; shown only after login
...
void MainWindow::onLoginSuccess(User* user, User** allUsers, int userCount)
{
    ...
    // Build fresh shell layout each time
...
void MainWindow::buildSidebar()
{
    ...
    // Brand — shows app name
    auto* brand = new QLabel(APP_NAME);
    brand->setStyleSheet(
        "font-size:21px; font-weight:800; color:#5555EE; "
        "letter-spacing:-0.8px; padding: 0 4px 14px 4px;");
    sl->addWidget(brand);
    ...
    QString ini = m_currentUser
        ? QString::fromStdString(m_currentUser->getUsername()).left(1).toUpper()
        : "?";
    userRow->addWidget(avatar(ini, 28));
    auto* uname = new QLabel(
...
{
    SidebarButton* btns[] = {
        m_btnFeed, m_btnNotifications,
        m_btnSearch, m_btnProfile, m_btnTimeSpent
    };
    for (auto* b : btns)
...
void MainWindow::onLogout()
{
    auto r = QMessageBox::question(this, APP_NAME,
        "Are you sure you want to log out?", QMessageBox::Yes | QMessageBox::No);
...
{
    // No confirmation — just switch directly
    if (m_currentUser) m_currentUser->logOut();
    if (m_timeSpentPage) m_timeSpentPage->stopSession();
    ...
    m_currentUser = nullptr;
    m_authPage->resetToLogin();   // clears fields + reloads users
    m_appShell->hide();
    m_authPage->show();
}
...
{
    // Stop timer before destroying widgets
    if (m_timeSpentPage) m_timeSpentPage->stopSession();

    if (m_appShell->layout()) {
        // Remove all child widgets from layout and schedule deletion
        QLayoutItem* item;
    ...
    // Null all pointers — deleteLater will clean memory asynchronously
